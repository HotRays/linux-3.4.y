...
static int ft_i2c_chip_id_read(u8 *rxdata, int length)
{
	int ret;
	struct i2c_msg msgs[] = {
		{
...
+*******************************************************/
static s8 ft_i2c_test(struct i2c_client *client)
{
	u8 test[5] = {0xA8};
	u8 retry = 0;
	s8 ret = -1;
	...
	while(retry++ < 5)
	{
		ret = ft_i2c_chip_id_read(test, 2);
		if (ret > 0)
		{
			printk("ft_i2c_test test[2]=%d\n",test[2]);
			return ret;
		}
		printk("ft i2c test failed time %d.",retry);
		msleep(10);
	}
	return ret;
}
...
ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	tp_data = client->dev.platform_data;
    ...
    input_set_abs_params(input_dev, ABS_MT_POSITION_X , 0, (tp_data)?tp_data->xres - 1:SCREEN_MAX_X-1, 0, 0);
    input_set_abs_params(input_dev, ABS_MT_POSITION_Y , 0, (tp_data)?tp_data->yres - 1:SCREEN_MAX_Y-1, 0, 0);
	...
	input_set_abs_params(input_dev, ABS_X, 0, (tp_data)? tp_data->xres:SCREEN_MAX_X, 0, 0);
	input_set_abs_params(input_dev, ABS_Y, 0, (tp_data)? tp_data->yres:SCREEN_MAX_Y, 0, 0);
