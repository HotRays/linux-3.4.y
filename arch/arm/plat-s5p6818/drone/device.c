...
int  nxpmac_init(struct platform_device *pdev)
{
         ...
         NX_RSTCON_SetnRST(RESETINDEX_OF_DWC_GMAC_MODULE_aresetn_i, RSTCON_NEGATE);
         udelay(100);
         NX_RSTCON_SetnRST(RESETINDEX_OF_DWC_GMAC_MODULE_aresetn_i, RSTCON_ASSERT);
         udelay(100);
         NX_RSTCON_SetnRST(RESETINDEX_OF_DWC_GMAC_MODULE_aresetn_i, RSTCON_NEGATE);
         udelay(100);
...
static struct i2c_board_info __initdata gt9xx_i2c_info_ = {
        .type   = "gt9xx_ts",
	.addr	= (0x14),
};
#endif
...
static struct gpio_led f6818_leds[] = {
	{
		.name = "led1",
...
static struct gpio_led_platform_data f6818_gpio_leds_platform_data = {
		.num_leds = ARRAY_SIZE(f6818_leds),
		.leds = f6818_leds,
};

static struct platform_device f6818_gpio_leds = {
		.name = "leds-gpio",
		.id = -1,
		.dev = {
				.platform_data = &f6818_gpio_leds_platform_data,	
		},
};
#endif
...
#if defined(CONFIG_TOUCHSCREEN_TSC2007)
    if(screen_type == 0 && screen_tp_type == 1){
...
#if defined(CONFIG_TOUCHSCREEN_FT5X0X)
	if(screen_type == 0 && screen_tp_type == 0){
			printk("plat: add touch(ft5x0x).2 device\n");
			i2c_register_board_info(2, &ft5x6_i2c_bdi, 1);
	} else if(screen_type == 2){
			printk("plat: add touch(ft5x0x).0 device\n");
...
#if defined(CONFIG_TOUCHSCREEN_GT9XX)
 if(screen_type == 0 && screen_tp_type == 0){
        i2c_register_board_info(2, &gt9xx_i2c_info_, 1);
        printk("plat: add touch(gt9xx).0 device\n");
   } else if(screen_type == 2){
        i2c_register_board_info(0, &gt9xx_i2c_info_, 1);
        printk("plat: add touch(gt9xx).2 device\n");
   }
#endif
	...
	platform_device_register(&f6818_gpio_leds);
#endif
