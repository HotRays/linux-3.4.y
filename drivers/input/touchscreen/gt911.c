...
static const char *goodix_ts_name = GOODIX_TS_NAME;
...
static void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    GTP_GPIO_OUTPUT(ts->gpio_irq, client->addr == 0x14);
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    GTP_DEBUG_FUNC();
    ret = GTP_GPIO_REQUEST(ts->gpio_irq, "GTP_INT_IRQ");
    if (ret < 0) 
    {
    ...
    {
        GTP_GPIO_AS_INT(ts->gpio_irq);  
        ts->client->irq = gpio_to_irq(ts->gpio_irq);
    }
    if(ts->gpio_rst == -1)
    {
	return 0;
