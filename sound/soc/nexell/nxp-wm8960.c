...
static struct snd_soc_codec *wm8960 = NULL;
...
{
	struct snd_soc_codec *codec = wm8960;
	pr_debug("%s\n", __func__);
#if 0
	snd_soc_update_bits(codec, WM8960_LOUT1_HP_CONTROL, 0x40, 0x40);
	snd_soc_update_bits(codec, WM8960_ROUT1_HP_CONTROL, 0x40, 0x40);
#endif
	return 0;
}
...
static int wm8960_dai_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_codec *codec = rtd->codec;
...
static struct snd_soc_ops wm8960_ops = {
...
static struct snd_soc_dai_link wm8960_dai_link = {
	...
	.codec_dai_name = "wm8960-hifi",		/* wm8960_dai's name */
	.codec_name 	= "wm8960.1-001a",		/* wm8960_i2c_driver name + '.' + bus + '-' + address(7bit) */
	.ops 			= &wm8960_ops,
	.symmetric_rates = 1,
	.init			= wm8960_dai_init,
	.ops 			= &wm8960_ops,
};

static struct snd_soc_card wm8960_card = {
	.name 			= "I2S-WM8960",		/* proc/asound/cards */
	.owner 			= THIS_MODULE,
	.dai_link 		= &wm8960_dai_link,
	.num_links 		= 1,
...
static int wm8960_probe(struct platform_device *pdev)
{
	...
	struct snd_soc_card *card = &wm8960_card;
	...
	pr_debug("wm8960-dai: register card %s -> %s\n",
...
static struct platform_driver wm8960_driver = {
	...
	.probe		= wm8960_probe,
	.remove		= __devexit_p(wm8960_remove),
};
module_platform_driver(wm8960_driver);

MODULE_AUTHOR("jhkim <jhkim@nexell.co.kr>");
MODULE_DESCRIPTION("Sound codec-wm8960 driver for the SLSI");
MODULE_LICENSE("GPL");
